  ...
  return (
    <div className='p-3.5 h-full overflow-auto bg-background'>
      ...
      {prodOpen && prod ? (
        <div
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setProdOpen(false);
          }}
        ...
        >
          <div className='w-[420px] max-w-[92vw] rounded-xl border border-border bg-secondary p-4'>
            ...
            <div className='text-sm leading-7'>
              <div>
                <b>제품코드</b> {prod.productCode ?? '-'}
              </div>
              <div>
                <b>제품명</b> {prod.productName ?? '-'}
              </div>
              <div>
                <b>주종</b> {toKoreanType(prod.type ?? '-')}
              </div>
              <div>
                <b>브랜드</b> {prod.brand ?? '-'}
              </div>
              <div>
                <b>구매단가</b>{' '}
                {prod.purchasePrice != null ? money(prod.purchasePrice) : '-'}
              </div>
              <div>
                <b>판매단가</b>{' '}
                {prod.salePrice != null ? money(prod.salePrice) : '-'}
              </div>
            </div>
