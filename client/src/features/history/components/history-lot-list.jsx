...
export function HistoryLotList({ rows, selectedLotId, onSelect }) {
  return (
    <div style={{ borderRight: '1px solid #ddd', height: '100%', overflow: 'auto' }}>
      <div style={{ padding: 10, fontWeight: 800, borderBottom: '1px solid #ddd' }}>거래일순</div>

      {rows.length === 0 ? (
        <div style={{ padding: 12, color: '#666', fontSize: 12 }}>데이터 없음</div>
      ) : rows.map((r) => {
        const active = Number(selectedLotId) === Number(r.lotId);
        const deltaText = formatDelta(r.status, r.totalDelta);
        const vendorText = r.status === 'INBOUND' ? r.vendorName : r.status === 'OUTBOUND' ? r.sellerVendorName : '';
          ...
          >
            ...
            <div style={{ marginTop: 6, display: 'flex', justifyContent: 'space-between' }}>
              ...
              <div style={{ fontSize: 12 }}>
                ...
                <span style={deltaStyle(r.status, r.totalDelta)}>{deltaText}</span>
              </div>
            </div>
