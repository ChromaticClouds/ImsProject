...
export function InboundCompletedRow(props) {
  ...
  const memoText = typeof row.memo === 'string' ? row.memo.trim() : '';
          ...
          <TooltipProvider>
            <Tooltip delayDuration={80}>
              <TooltipTrigger asChild>
                <Badge
                  variant="secondary"
                  className="bg-amber-300 text-amber-700"
                >
                  변경됨
                </Badge>
              </TooltipTrigger>

              <TooltipContent className="max-w-[320px] bg-amber-400">
                <div className="text-xs font-semibold mb-1">메모</div>
                <div className="text-sm">{hasMemo ? memoText : '-'}</div>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ) : null}
