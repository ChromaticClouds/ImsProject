 ...
 * }} props
 */
export function OutboundDateRangePicker({ value, onChange, disabled }) {
  ...
  const label = value?.from && value?.to ? `${value.from} ~ ${value.to}` : '기간 선택';
  ...
  function handleSelect(next) {
    ...
    if (!next?.from || !next?.to) {
      onChange({
        ...
        to: next?.to ? toYMD(next.to) : value.to,
      });
      return;
    }
