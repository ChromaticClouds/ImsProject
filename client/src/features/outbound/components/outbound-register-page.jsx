...
export function OutboundRegisterPage() {
  ...
  async function handleComplete() {
    ...
    if (hasShortage)
      return setError('재고 부족 품목이 있어 출고 처리할 수 없습니다.');
        ...
        <div className='mb-5 flex flex-wrap items-start justify-between gap-3'>
          <div>
            ...
            <div className='mt-2 text-sm text-muted-foreground'>
              <span className='font-medium text-foreground'>판매처</span>{' '}
              {sellerVendorName}
              <span className='mx-2'>·</span>
              <span className='font-medium text-foreground'>
                출고 일자
              </span>{' '}
              {formatKoreanDateTime(shippedAt)}
            </div>
          </div>
            ...
            <div className='sticky top-5 space-y-4'>
              ...
              <div className='rounded-2xl border bg-card p-5'>
                <div className='mb-2 flex items-center justify-between'>
                  ...
                  <div
                    className={`text-xs ${memoOver ? 'text-destructive' : 'text-muted-foreground'}`}
                  >
