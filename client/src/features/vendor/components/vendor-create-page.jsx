...
export function VendorCreatePage() {
  ...
  const errors = useMemo(() => {
    ...
    else if (ceo.length < 2 || ceo.length > 10)
      e.대표자명 = '대표자명은 2~10자만 허용됩니다';
  ...
  const isValidItemsForSupplier = useMemo(() => {
    ...
    return selectedItems.every(
      (x) => Number.isFinite(x.unitPrice) && x.unitPrice > 0,
    );
        ...
        <div className='mb-5 flex flex-wrap items-start justify-between gap-3'>
          <div>
            ...
            <div className='mt-1 text-sm text-muted-foreground'>
              거래처 기본정보를 입력하고, 공급처(Supplier)인 경우 품목/단가를
              설정하세요.
            </div>
          </div>
        </div>
