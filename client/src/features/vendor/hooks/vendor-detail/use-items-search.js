...
// @ts-check
import { useQuery } from '@tanstack/react-query';
import { fetchProducts } from '@/features/vendor/api/index';

/**
 * @param {{ keyword: string, excludeAssigned?: boolean, currentVendorId?: number, enabled?: boolean }} params
 */
export function useItemsSearch(params) {
  const keyword = params.keyword?.trim() ?? '';
  const excludeAssigned = params.excludeAssigned ?? true;
  const currentVendorId = params.currentVendorId;
  const enabled = params.enabled ?? keyword.length > 0;
