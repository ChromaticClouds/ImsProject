    ...
    public String selectCompletedItemsByOrderNumber(Map<String, Object> params) {
      return """
        SELECT
          ...
          -- ✅ 이번 입고 lot 기준 실제 입고수량 (없으면 fallback: 발주수량)
          COALESCE(SUM(h.after_count - h.before_count), o.`count`) AS orderQty,

          vi.purchase_price AS purchasePrice,

          -- ✅ 이번 입고 lot 기준 금액
          COALESCE(SUM(h.after_count - h.before_count), o.`count`) * vi.purchase_price AS lineAmount,

          p.image_url AS imageUrl,
          p.sale_price AS salePrice,

          -- (옵션) 현재고도 같이 주고 싶으면 아래 조인 + 컬럼 살려
          IFNULL(s.`count`, 0) AS currentStock
        ...
        GROUP BY
          o.id, o.vendor_item_id, vi.product_id, p.name, p.type, p.brand,
          o.`count`, vi.purchase_price, p.image_url, p.sale_price, s.`count`
...
//    }
 ...
 // ✅ qty_changed만 찍는 전용
    public String markQtyChangedByOrderId() {
      return new SQL(){{
        UPDATE("`orders`");
        SET("qty_changed = 1");
        WHERE("id = #{orderId}");
    ...
    // ✅ 현재 orders.count(발주수량) 조회
    public String selectOrderCountById() {
      return new SQL(){{
        SELECT("o.`count`");
        FROM("`orders` o");
        WHERE("o.id = #{orderId}");
