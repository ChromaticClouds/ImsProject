package com.example.ims.features.statistics.service;
...
import com.example.ims.features.statistics.mapper.StatisticsMapper;
...
public class StatisticsService {
  ...
  public List<String> getBrandsByType(String type) {
    if (!StringUtils.hasText(type)) throw new IllegalArgumentException("type 필수");
  ...
  private void validateRange(LocalDate from, LocalDate to) {
    if (from == null || to == null) throw new IllegalArgumentException("기간 필수");
    if (from.isAfter(to)) throw new IllegalArgumentException("시작일이 종료일보다 뒤일 수 없음");
    if (to.isAfter(LocalDate.now())) throw new IllegalArgumentException("미래 날짜 선택 불가");
    if (from.plusYears(1).isBefore(to)) throw new IllegalArgumentException("날짜 설정 범위 한도 초과입니다.");
  }
